...
cfg = get_config()
cfg.merge_from_file("/data/zengsheng/works/work1/eval_algorithms/yolov5_object_tracking/deep_sort/configs/deep_sort.yaml")
...
def plot_bboxes(image, bboxes, line_thickness=None):
    ...
    for (x1, y1, x2, y2, cls_id, pos_id) in bboxes:
        ...
        # cv2.rectangle(image, c1, c2, color, tf)  # filled
        # cv2.putText(image, '{} ID-{}'.format(cls_id, pos_id), (c1[0], c1[1] - 2), 0, tl / 3,
        #             [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        cv2.putText(image, '{}'.format(pos_id), (c1[0], c1[1] - 1), 0, tl / 3,
                    [0, 0, 255], thickness=tf)
    return image
